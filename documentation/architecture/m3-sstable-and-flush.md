...
M3 persists the memtable: when the active memtable fills, it is written to an **immutable
on-disk SSTable** and the WAL segment that covered it is reclaimed. The engine now has durable,
sorted tables, and reads span the memtable set and those tables — the shape every later storage
milestone extends. What M3 hands to the rest of the project:
...
2. **Flush with the D3 ordering** — the table is fsync'd and atomically renamed **before** the
   ...
   what M5's manifest formalises.
3. **Discovery and file numbers** — `NNNNNN.sst` files share one monotonic file-number counter
   with WAL segments; tables are found by directory scan (E2, an interim the manifest at M5
   replaces).

Where it sits: M3 reads what M2 left accumulating in memory, writes M1's durable-logged data to
a new on-disk format (a `Format-Change:` snapshot, pinned by a golden file), and hands M4 the
task of merging many tables while M5 takes over tracking them. Flush is synchronous at M3
(background flush + write stalls are M6); blocks are uncompressed (a later, benchmark-gated
change).
...
The engine-level view of M3: when and how a full memtable becomes a durable table without ever
losing an acknowledged write, and how reads and recovery now consult both memory and disk. The
byte layout and the block reader those flows lean on are the LLD in §3.

### 2.1 Flush: memtable → SSTable, then reclaim the WAL segment
...
### 2.2 Reads across memtables and tables
...
### 2.3 Recovery: replay, flush, then reset the log
...
Where the HLD flows become concrete: the block-table layout, how phrase compression and
restart points make a block both compact and seekable, and the types that build and read it.

### 3.1 The SSTable file — a LevelDB block table
...
### 3.2 Reading a table: index → data block → restart search
