...
- **M5.5 — Concurrent write path.** Bounded writer cohorts, shared force outside state locks, ordered memtable publication preserving ADR-0008, and background flush with bounded queues and write stalls. *Depends on M5; blocks M6.* See the [granular plan](m5.5-concurrent-write-path.md), including decisions to review before implementation.
